...
export async function GET(request) {
  try {
    ...
    const [statsResult, admissionsResult] = await Promise.all([
      query(`
        SELECT 
    ...
    const totalCount = admissions.length > 0 ? admissions[0].total_count : 0;
    admissions.forEach(r => delete r.total_count);
